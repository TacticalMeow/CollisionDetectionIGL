...
//		else
//		{
//			scn->data().MyRotate(Eigen::Vector3d(1, 0, 0) ,yrel / 100);
//			scn->data().MyRotate(Eigen::Vector3d(0, 1, 0) , xrel / 100);
//
//		}
//	}
//	
...
//		else
//		{
//			scn->MyRotate(Eigen::Vector3d(1, 0, 0), yrel / 100);
//			scn->MyRotate(Eigen::Vector3d(0, 1, 0), xrel / 100);
//
//		}
//	}
//}
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked)
	{
		...
		if (button == 1)
		{
			...
			double xToMove = -(double)xrel / core().viewport[3] * (z + 2 * near) * (far) / (far + 2 * near) * 2.0 * tanf(angle / 360 * M_PI) / (core().camera_zoom * core().camera_base_zoom);
			double yToMove = (double)yrel / core().viewport[3] * (z + 2 * near) * (far) / (far + 2 * near) * 2.0 * tanf(angle / 360 * M_PI) / (core().camera_zoom * core().camera_base_zoom);
			//We get the scene rotation matrix transpose in order to get the correct direction of object translation
			Eigen::Matrix3d Mtx_ScnRotation = scn->GetRotation().transpose();
			scn->data().MyTranslate(Mtx_ScnRotation * Eigen::Vector3d(xToMove, 0, 0), true);
			scn->data().MyTranslate(Mtx_ScnRotation * Eigen::Vector3d(0, yToMove, 0), true);
			scn->WhenTranslate();
		}
		else
		{
			scn->data().RotateInSystem(Eigen::Vector3d(1, 0, 0), -(double)yrel / core().viewport[3] * (z + 2 * near) * (far) / (far + 2 * near) * 2.0 * tanf(angle / 360 * M_PI) / (core().camera_zoom * core().camera_base_zoom));
			scn->data().RotateInSystem(Eigen::Vector3d(0, 1, 0), -(double)xrel / core().viewport[3] * (z + 2 * near) * (far) / (far + 2 * near) * 2.0 * tanf(angle / 360 * M_PI) / (core().camera_zoom * core().camera_base_zoom));
		}
	}
	else
	{
		...
		else
		{
			...
			scn->RotateInSystem(Eigen::Vector3d(1, 0, 0), 2.0f*(-(double)yrel / (double)core().viewport[3]));
			scn->RotateInSystem(Eigen::Vector3d(0, 1, 0), 2.0f * (-(double)xrel / (double)core().viewport[3]));
		}
	}
}
